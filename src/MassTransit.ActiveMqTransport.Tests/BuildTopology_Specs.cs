...
namespace MassTransit.ActiveMqTransport.Tests
{
    ...
    public class Using_topology_to_bind_consumers
    {
        ...
        {
            _consumeTopology.GetMessageTopology<SecondInterface>().Bind();
            ...
            Assert.That(topology.Topics.Any(x => x.EntityName == $"VirtualTopic.{interfaceName}"), Is.True);
            //Assert.That(
            //    topology.Queues.Any(x => x.Source.TopicDescription.Path == interfaceName && x.Destination.QueueDescription.Path == _inputQueueName),
            //    Is.True);

            interfaceName = _nameFormatter.GetMessageName(typeof(FirstInterface)).ToString();

            Assert.That(topology.Topics.Any(x => x.EntityName == $"VirtualTopic.{interfaceName}"), Is.False);
            //Assert.That(
        ...
        {
            _consumeTopology.GetMessageTopology<SingleInterface>().Bind();
            ...
            Assert.That(topology.Topics.Any(x => x.EntityName == $"VirtualTopic.{singleInterfaceName}"), Is.True);
            //Assert.That(
        ...
        public void Setup()
        {
            _nameFormatter = new ActiveMqMessageNameFormatter();
            _entityNameFormatter = new MessageNameFormatterEntityNameFormatter(_nameFormatter);
            _consumeTopology = new ActiveMqConsumeTopology(ActiveMqBusFactory.MessageTopology, new ActiveMqPublishTopology(ActiveMqBusFactory.MessageTopology));
        ...
        ActiveMqMessageNameFormatter _nameFormatter;
        MessageNameFormatterEntityNameFormatter _entityNameFormatter;
        IActiveMqConsumeTopologyConfigurator _consumeTopology;
    ...
    public class Using_flattened_topology_to_bind_publishers
    {
        ...
        public void Should_include_a_binding_for_the_second_interface_only()
        {
            ...
            Assert.That(topology.Queues.Length, Is.EqualTo(1));
            //Assert.That(
        ...
        public void Should_include_a_binding_for_the_single_interface()
        {
            ...
            Assert.That(topology.Queues.Length, Is.EqualTo(0));
        }
        ...
        public void Setup()
        {
            _nameFormatter = new ActiveMqMessageNameFormatter();
            _entityNameFormatter = new MessageNameFormatterEntityNameFormatter(_nameFormatter);
            _publishTopology = new ActiveMqPublishTopology(ActiveMqBusFactory.MessageTopology);
        ...
        ActiveMqMessageNameFormatter _nameFormatter;
        MessageNameFormatterEntityNameFormatter _entityNameFormatter;
        IActiveMqPublishTopologyConfigurator _publishTopology;
